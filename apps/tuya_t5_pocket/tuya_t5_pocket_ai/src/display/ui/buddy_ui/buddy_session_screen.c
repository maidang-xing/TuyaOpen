...
/**
 * @brief Format a wall-clock time from the BLE state into HH:MM.
 *
...
STATIC VOID_T __init(VOID_T)
{
    ...
    /* 2. Get a fresh BLE snapshot + ask the daemon for an immediate refresh
     *    so live session metadata (running flag, ctx, entries) is current. */
    buddy_ble_snapshot(&s_state);
    (VOID_T)buddy_ble_send_hb_req("session");
