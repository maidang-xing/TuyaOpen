/**
 * @file persona_snail.c
 * @brief "snail" ASCII 人格状态机。
 *
 * 由 tools/port_buddies.py 从 claude-desktop-buddy/src/buddies/snail.cpp
 * 机械化迁移而来。为保持与上游 1:1 对应，sprite 数据、tick 序列与 overlay
 * 布局一律保留；RGB565 颜色参数在单色 OLED 上被实现忽略。
 *
 * 七个状态函数（对应 buddy_persona_state_e）：
 *   __snail_sleep / __snail_idle / __snail_busy / __snail_attention /
 *   __snail_celebrate / __snail_dizzy / __snail_heart
 *
...
static void __snail_sleep(uint32_t t) {
  ...
  ascii_print_sprite(P[SEQ[beat]], 5, 0, 0xD8FE, 0);
  ...
  ascii_set_cursor(BUDDY_X_CENTER + 8 - p1, BUDDY_Y_OVERLAY + 14 - p1);
  ascii_print("z");
  ascii_set_color(BUDDY_WHITE);
  ascii_set_cursor(BUDDY_X_CENTER + 14 - p2, BUDDY_Y_OVERLAY + 10 - p2);
  ascii_print("Z");
  ascii_set_color(BUDDY_DIM);
  ascii_set_cursor(BUDDY_X_CENTER + 4 - p3 / 2, BUDDY_Y_OVERLAY + 6 - p3 / 2);
  ascii_print("z");
}

// ─── IDLE ───  ~14s cycle, 10 poses — antennae wiggle, eye peeks, slime trail
static void __snail_idle(uint32_t t) {
  ...
  if ((t / 6) & 1) {
    ...
    ascii_set_cursor(BUDDY_X_CENTER - 10, BUDDY_Y_BASE + 32);
    ascii_print(".");
  }
}

// ─── BUSY ───  ~10s cycle, 6 poses — shell spiral spinning, focused eyes, dot ticker
static void __snail_busy(uint32_t t) {
...
static void __snail_attention(uint32_t t) {
  ...
  ascii_print_sprite(P[pose], 5, 0, 0xD8FE, xOff);
...
static void __snail_celebrate(uint32_t t) {
  ...
  ascii_print_sprite(P[SEQ[beat]], 5, Y_SHIFT[beat], 0xD8FE, 0);
...
static void __snail_dizzy(uint32_t t) {
  ...
  ascii_print_sprite(P[SEQ[beat]], 5, 0, 0xD8FE, X_SHIFT[beat]);
...
static void __snail_heart(uint32_t t) {
  ...
  uint8_t beat = (t / 5) % sizeof(SEQ);
  ascii_print_sprite(P[SEQ[beat]], 5, Y_BOB[beat], 0xD8FE, 0);
 ...
 * Registration
 * --------------------------------------------------------------------------- */
const ascii_persona_t PERSONA_SNAIL = {
    .name = "snail",
    .states = { __snail_sleep, __snail_idle, __snail_busy, __snail_attention, __snail_celebrate, __snail_dizzy, __snail_heart },
};
