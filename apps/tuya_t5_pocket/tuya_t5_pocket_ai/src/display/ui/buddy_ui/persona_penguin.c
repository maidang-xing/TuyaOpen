/**
 * @file persona_penguin.c
 * @brief "penguin" ASCII 人格状态机。
 *
 * 由 tools/port_buddies.py 从 claude-desktop-buddy/src/buddies/penguin.cpp
 * 机械化迁移而来。为保持与上游 1:1 对应，sprite 数据、tick 序列与 overlay
 * 布局一律保留；RGB565 颜色参数在单色 OLED 上被实现忽略。
 *
 * 七个状态函数（对应 buddy_persona_state_e）：
 *   __penguin_sleep / __penguin_idle / __penguin_busy / __penguin_attention /
 *   __penguin_celebrate / __penguin_dizzy / __penguin_heart
 *
...
static void __penguin_sleep(uint32_t t) {
  ...
  ascii_print_sprite(P[SEQ[beat]], 5, 0, 0x041F, 0);
...
static void __penguin_idle(uint32_t t) {
...
static void __penguin_busy(uint32_t t) {
...
static void __penguin_attention(uint32_t t) {
  ...
  ascii_print_sprite(P[pose], 5, 0, 0x041F, xOff);
...
static void __penguin_celebrate(uint32_t t) {
  ...
  ascii_print_sprite(P[SEQ[beat]], 5, Y_SHIFT[beat], 0x041F, 0);
...
static void __penguin_dizzy(uint32_t t) {
  ...
  ascii_print_sprite(P[SEQ[beat]], 5, 0, 0x041F, X_SHIFT[beat]);
...
static void __penguin_heart(uint32_t t) {
  ...
  uint8_t beat = (t / 5) % sizeof(SEQ);
  ascii_print_sprite(P[SEQ[beat]], 5, Y_BOB[beat], 0x041F, 0);
 ...
 * Registration
 * --------------------------------------------------------------------------- */
const ascii_persona_t PERSONA_PENGUIN = {
    .name = "penguin",
    .states = { __penguin_sleep, __penguin_idle, __penguin_busy, __penguin_attention, __penguin_celebrate, __penguin_dizzy, __penguin_heart },
};
