...
static void pet_movement_cb(lv_timer_t *timer)
{
    ...
    if (!data->pet_is_walking) {
        ...
        if (data->idle_animation_timer >= data->idle_animation_duration) {
            ...
            data->idle_animation_timer = 0;
            data->idle_animation_duration = PET_IDLE_ANIMATION_SWITCH_MIN +
    ...
    if (data->pet_state_timer >= data->pet_state_duration) {
        ...
        if (data->pet_is_walking) {
            ...
            data->pet_state_duration =
                PET_WALK_DURATION_MIN + (rand() % (PET_WALK_DURATION_MAX - PET_WALK_DURATION_MIN));
            ...
            data->pet_state_duration =
                PET_IDLE_DURATION_MIN + (rand() % (PET_IDLE_DURATION_MAX - PET_IDLE_DURATION_MIN));
