...
static const int s_pet_opt_values[PET_OPT_TOTAL][PET_STATE_TOTAL] = {
    ...
    [PET_EVENT_FEED_HAMBURGER] = {-1, 8, -1, 0}, [PET_EVENT_DRINK_WATER] = {1, 2, -2, 1},
    [PET_EVENT_FEED_PIZZA] = {-1, 6, -3, 2},     [PET_EVENT_FEED_APPLE] = {1, 1, 0, 1},
    [PET_EVENT_FEED_FISH] = {1, 3, -1, 0},       [PET_EVENT_FEED_CARROT] = {2, 1, 0, -2},
    [PET_EVENT_FEED_ICE_CREAM] = {0, 3, -2, 3},  [PET_EVENT_FEED_COOKIE] = {0, 3, -2, 0},
    [PET_EVENT_TOILET] = {0, -1, -3, 1},         [PET_EVENT_TAKE_BATH] = {0, -2, 10, 3},
    [PET_EVENT_SEE_DOCTOR] = {10, -1, -2, -5},   [PET_EVENT_SLEEP] = {3, 10, 0, 1},
    [PET_EVENT_WAKE_UP] = {1, 10, -2, 2},        [PET_STAT_RANDOMIZE] = {0, 0, 0, 0},
    [PET_EVENT_TIMER] = {-1, -3, -2, -4}};

static const int s_pet_dp_values[PET_STATE_TOTAL] = {[PET_S_HEALTH_INDEX] = DPID_HEALTH,
...
OPERATE_RET game_pet_show(int *state)
{
    ...
    PR_INFO("Game Pet State - Health: %d, Energy: %d, Cleaness: %d, Happiness: %d", state[PET_S_HEALTH_INDEX],
            state[PET_S_ENERGY_INDEX], state[PET_S_CLEAN_INDEX], state[PET_S_HAPPINESS_INDEX]);
    ...
    // report mood dp
    dp_obj_t dps = {.id = DPID_MOOD, .type = PROP_ENUM, .value = (dp_value_t)(uint32_t)s_pet_mood_dp_value};
...
{
    return tal_kv_set(KVKEY_GAME_PET_STATE, (const uint8_t *)state, sizeof(int) * PET_STATE_TOTAL);
}
...
{
    pet_stats_t menu_state = {.health = state[PET_S_HEALTH_INDEX],
