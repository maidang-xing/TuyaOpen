/**
 * @file persona_robot.c
 * @brief "robot" ASCII 人格状态机。
 *
 * 由 tools/port_buddies.py 从 claude-desktop-buddy/src/buddies/robot.cpp
 * 机械化迁移而来。为保持与上游 1:1 对应，sprite 数据、tick 序列与 overlay
 * 布局一律保留；RGB565 颜色参数在单色 OLED 上被实现忽略。
 *
 * 七个状态函数（对应 buddy_persona_state_e）：
 *   __robot_sleep / __robot_idle / __robot_busy / __robot_attention /
 *   __robot_celebrate / __robot_dizzy / __robot_heart
 *
...
static void __robot_sleep(uint32_t t) {
  ...
  ascii_print_sprite(P[SEQ[beat]], 5, 0, 0xC618, 0);
...
static void __robot_idle(uint32_t t) {
  ...
  if ((t / 4) & 1) {
    ...
    ascii_set_cursor(BUDDY_X_CENTER - 1, BUDDY_Y_BASE - 4);
    ascii_print(".");
  }
}

// ─── BUSY ───  ~10s cycle, 6 poses + binary stream
static void __robot_busy(uint32_t t) {
  ...
  ascii_print(BITS[t % 6]);
}

// ─── ATTENTION ───  ~8s cycle, 6 poses + ! pulse
static void __robot_attention(uint32_t t) {
  ...
  ascii_print_sprite(P[pose], 5, 0, 0xC618, xOff);
...
static void __robot_celebrate(uint32_t t) {
  ...
  ascii_print_sprite(P[SEQ[beat]], 5, Y_SHIFT[beat], 0xC618, 0);
...
static void __robot_dizzy(uint32_t t) {
  ...
  ascii_print_sprite(P[SEQ[beat]], 5, 0, 0xC618, X_SHIFT[beat]);
...
static void __robot_heart(uint32_t t) {
  ...
  uint8_t beat = (t / 5) % sizeof(SEQ);
  ascii_print_sprite(P[SEQ[beat]], 5, Y_BOB[beat], 0xC618, 0);
 ...
 * Registration
 * --------------------------------------------------------------------------- */
const ascii_persona_t PERSONA_ROBOT = {
    .name = "robot",
    .states = { __robot_sleep, __robot_idle, __robot_busy, __robot_attention, __robot_celebrate, __robot_dizzy, __robot_heart },
};
