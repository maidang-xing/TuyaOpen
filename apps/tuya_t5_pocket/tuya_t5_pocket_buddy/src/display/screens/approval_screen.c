/**
 * @file approval_screen.c
 * @brief Permission approval standalone screen (B&W, 4-line info).
 *
 * Info area (4 lines, no overlap):
 *   Tool    - tool name (FONT_M, prominent)
 *   Info    - parameters/path (FONT_S)
 *   Session - session name (first user prompt, FONT_S)
 *   Owner   - owner + model name (FONT_S)
 *
 * LED behavior:
 *   - On arrival, buddy_main_screen sets fast-blink (notify user)
 *   - __screen_init() turns LED off (user has seen the screen)
 *   - __send_and_back() also ensures LED is off after decision
 *
 * Layout (384x168, no footer):
 *   title_bar  0-20   black bg white text "PERMISSION REQUEST"
 *   tool       24     FONT_M h=16
 *   info       43     FONT_S h=14
 *   session    60     FONT_S h=14
 *   owner      77     FONT_S h=14
 *   separator  94     1px black line
...
STATIC VOID_T __screen_init(VOID_T)
{
    ...
    /* Populate info from state snapshot */
    buddy_state_snapshot(&s_snap);
    s_cursor = 0;
...
STATIC VOID_T __send_and_back(const char *decision)
{
    ...
    (VOID_T)buddy_ws_send_permission(s_snap.prompt_id, decision);
