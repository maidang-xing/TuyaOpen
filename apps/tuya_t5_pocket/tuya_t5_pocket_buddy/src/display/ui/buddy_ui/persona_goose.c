/**
 * @file persona_goose.c
 * @brief "goose" ASCII 人格状态机。
 *
 * 由 tools/port_buddies.py 从 claude-desktop-buddy/src/buddies/goose.cpp
 * 机械化迁移而来。为保持与上游 1:1 对应，sprite 数据、tick 序列与 overlay
 * 布局一律保留；RGB565 颜色参数在单色 OLED 上被实现忽略。
 *
 * 七个状态函数（对应 buddy_persona_state_e）：
 *   __goose_sleep / __goose_idle / __goose_busy / __goose_attention /
 *   __goose_celebrate / __goose_dizzy / __goose_heart
 *
...
static void __goose_sleep(uint32_t t) {
...
static void __goose_idle(uint32_t t) {
...
static void __goose_busy(uint32_t t) {
  ...
  ascii_print(HONKS[t % 8]);
}

// ─── ATTENTION ───  ~8s cycle, 6 poses + ! pulse + HONK overlay
static void __goose_attention(uint32_t t) {
  ...
  if ((t / 4) & 1) {
    ascii_set_color(BUDDY_RED);
    ascii_set_cursor(BUDDY_X_CENTER + 14, BUDDY_Y_OVERLAY + 8);
    ascii_print("!");
  }
}

// ─── CELEBRATE ───  ~5.6s cycle, 6 poses + confetti rain + HONK
static void __goose_celebrate(uint32_t t) {
...
static void __goose_dizzy(uint32_t t) {
...
static void __goose_heart(uint32_t t) {
 ...
 * Registration
 * --------------------------------------------------------------------------- */
const ascii_persona_t PERSONA_GOOSE = {
    .name = "goose",
    .states = { __goose_sleep, __goose_idle, __goose_busy, __goose_attention, __goose_celebrate, __goose_dizzy, __goose_heart },
};
