/**
 * @file buddy_pie_screen.c
 * @brief Claude Desktop Buddy — Tab 4 "Model Usage" pie chart screen.
 *
 * dev_4_27 §UI 更新 §2 — render an actual pie chart (not horizontal bars).
...
/* ---------------------------------------------------------------------------
 * Refresh header (clock + WiFi/BLE/total tokens)
 * --------------------------------------------------------------------------- */
STATIC VOID_T __refresh_header(VOID_T)
{
    if (s_lbl_clock) {
    ...
    if (s_lbl_right) {
        const char *wifi = buddy_ble_cloud_is_connected() ? LV_SYMBOL_WIFI      : " ";
        const char *bt   = s_state.ble_connected          ? LV_SYMBOL_BLUETOOTH : " ";
        char tok[12];
...
STATIC VOID_T __init(VOID_T)
{
    ...
    (VOID_T)buddy_ble_send_hb_req("pie");
