 ...
 *                     Right:  "W B 100%"  (W=wifi, B/b=BLE, batt placeholder)
 *
 ...
 *   Applies live wall-clock delta so the time advances even without new BLE
...
/* ---------------------------------------------------------------------------
 * Header refresh — updates clock and BLE status indicator
 * --------------------------------------------------------------------------- */
STATIC VOID_T __refresh_header(VOID_T)
{
    /* Clock */
...
STATIC VOID_T __init(VOID_T)
{
    ...
    /* Snapshot BLE state first so builders can query it; then ask the
     * daemon for a fresh heartbeat so version/cost/token totals are
     * current the moment this screen comes up. */
    buddy_ble_snapshot(&s_state);
    (VOID_T)buddy_ble_send_hb_req("status");
