...
// static void create_sub_menu_with_items(menu_system_data_t *data, const char *title, const char *symbols[], const char
// *items[], uint8_t item_count);
...
lv_obj_t *menu_system_create_bottom_menu(lv_obj_t *parent)
{
    ...
    const lv_img_dsc_t *menu_icons[] = {&info_icon, &eat_icon, &toilet_icon, &sick_icon, &sleep_icon, &camera_icon};
    ...
    for (int i = 0; i < MENU_BUTTON_COUNT; i++) {
        ...
        lv_obj_align(data->menu_buttons[i], LV_ALIGN_BOTTOM_RIGHT, -(MENU_BUTTON_START_X - i * MENU_BUTTON_SPACING), 0);
...
void menu_system_update_pet_stats_for_testing(void)
{
    ...
    printf("Pet stats updated - Health: %d, Hungry: %d, Happy: %d, Age: %d days, Weight: %.1f kg\n",
           data->pet_stats.health, data->pet_stats.hungry, data->pet_stats.happy, data->pet_stats.age_days,
           data->pet_stats.weight_kg);
}
