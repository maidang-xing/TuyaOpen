...
/* Optional NUS extension co-registers its own primary service alongside Tuya's,
 * so the GATT DB holds both Tuya provisioning and NUS from boot. */
#define TAL_COMMON_SERVICE_MAX_NUM (2)
 ...
 * Tuya's ble_mgr; an optional NUS module registers an auxiliary callback here
 * so it can observe/consume GATT traffic targeted at the NUS service without
 * disturbing the Tuya pairing protocol. */
 ...
 *       observes traffic. This is used to let an optional NUS application
 *       drain its GATT writes/notifies without racing Tuya's ble_mgr.
 */
...
/**
 * @brief Return the handles assigned by the GATT stack to the NUS RX / TX
 *        characteristics (valid after tal_ble_bt_init returned OK).
    ...
    if ((role & TAL_BLE_ROLE_PERIPERAL) == TAL_BLE_ROLE_PERIPERAL) {
        ...
        *(ptkl_ble_service_char + TAL_COMMON_NOTIFY_CHAR_INDEX) = (TKL_BLE_CHAR_PARAMS_T){
...
#if (defined(ENABLE_CLAUDE_DESKTOP_BUDDY_BLE) && (ENABLE_CLAUDE_DESKTOP_BUDDY_BLE == 1))
        {
            ...
            /* NUS RX characteristic (central -> device). */
            *(ptkl_ble_service_char + TAL_CLAUDE_RX_CHAR_INDEX) = (TKL_BLE_CHAR_PARAMS_T){
            ...
            /* NUS TX characteristic (device -> central, notify). */
            *(ptkl_ble_service_char + TAL_CLAUDE_TX_CHAR_INDEX) = (TKL_BLE_CHAR_PARAMS_T){
            ...
            /* NUS service (secondary entry in the service table). */
