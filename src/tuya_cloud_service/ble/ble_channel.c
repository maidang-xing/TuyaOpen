...
static ble_channel_mgr_t s_ble_channel_mgr;
...
{
    if (s_ble_channel_mgr.rsp_data != NULL) {
...
{
    if (s_ble_channel_mgr.rsp_data == NULL || s_ble_channel_mgr.subpack_sent > s_ble_channel_mgr.subpack_len ||
        (s_ble_channel_mgr.subpack_sent == s_ble_channel_mgr.subpack_len && s_ble_channel_mgr.subpack_no != 0)) {
        return;
    }
    ...
    packet[offset++] = 0;
    packet[offset++] = 3;
    if (ble_channel_varint_encode(s_ble_channel_mgr.subpack_no, packet, TUYA_BLE_TRANS_DATA_SUBPACK_LEN, &offset) !=
        OPRT_OK) {
        tal_free(packet);
        return;
    }
    if (s_ble_channel_mgr.subpack_no == 0) {
        if (ble_channel_varint_encode(s_ble_channel_mgr.subpack_len, packet, TUYA_BLE_TRANS_DATA_SUBPACK_LEN, &offset) !=
                OPRT_OK ||
    ...
    uint32_t remaining = s_ble_channel_mgr.subpack_len - s_ble_channel_mgr.subpack_sent;
    uint32_t available = TUYA_BLE_TRANS_DATA_SUBPACK_LEN - offset;
    uint32_t data_len  = remaining < available ? remaining : available;
    if (data_len != 0) {
        memcpy(packet + offset, s_ble_channel_mgr.subpack_data + s_ble_channel_mgr.subpack_sent, data_len);
    }
    tuya_ble_send(type, 0, packet, offset + data_len);
    s_ble_channel_mgr.subpack_sent += data_len;
