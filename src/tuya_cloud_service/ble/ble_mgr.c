...
static bool ble_security_packet_allowed(bool bound, bool paired, uint16_t type, uint8_t mode)
{ return !(bound && mode == ENCRYPTION_MODE_NONE) && (paired || type == FRM_QRY_DEV_INFO_REQ || type == FRM_PAIR_REQ); }
static uint32_t ble_mgr_u32_from_big_endian(const uint8_t data[4])
{ return ((uint32_t)data[0] << 24) | ((uint32_t)data[1] << 16) | ((uint32_t)data[2] << 8) | data[3]; }
static int ble_mgr_packet_length_parse(const uint8_t *data, uint16_t len, uint16_t *out)
{ if (data == NULL || len < 2 || out == NULL) return OPRT_INVALID_PARM; uint16_t n = ((uint16_t)data[0] << 8) | data[1]; if (n < 10 || n > TUYA_BLE_AIR_FRAME_MAX) return OPRT_INVALID_PARM; *out = n; return OPRT_OK; }
static int ble_mgr_subpkg_resize(uint8_t **buf, uint32_t *cap, uint16_t n)
{ if (buf == NULL || *buf == NULL || cap == NULL || n < 10 || n > TUYA_BLE_AIR_FRAME_MAX) return OPRT_INVALID_PARM; if (*cap == n) return OPRT_OK; uint8_t *p = tal_malloc(n); if (p == NULL) return OPRT_MALLOC_FAILED; memset(p, 0, n); tal_free(*buf); *buf = p; *cap = n; return OPRT_OK; }
static bool ble_mgr_dev_info_buffer_valid(const void *mgr, const uint8_t *buf, uint32_t cap)
{ return mgr != NULL && buf != NULL && cap >= 64; }
