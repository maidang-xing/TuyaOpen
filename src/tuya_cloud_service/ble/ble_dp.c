...
static bool ble_dp_v4_payload_valid(const uint8_t *data, uint16_t len) { return data != NULL && len >= 5; }
static bool ble_dp_klv_length_valid(uint8_t type, uint16_t len)
{ if (type == BLE_DP_TYPE_RAW || type == BLE_DP_TYPE_STRING) return len <= 255; if (type == BLE_DP_TYPE_BOOL) return len == 1; if (type == BLE_DP_TYPE_VALUE) return len == 4; return (type == BLE_DP_TYPE_ENUM || type == BLE_DP_TYPE_BITMAP) && (len == 1 || len == 2 || len == 4); }
static bool ble_dp_enum_value_valid(uint32_t value, int count, char *const *values) { return count > 0 && values != NULL && value < (uint32_t)count && values[value] != NULL; }
static bool ble_dp_scalar_to_big_endian(const void *data, uint16_t len, uint8_t *output)
{ if (data == NULL || output == NULL || (len != 1 && len != 2 && len != 4)) return false; uint32_t value = 0; if (len == 1) value = *(const uint8_t *)data; else if (len == 2) { uint16_t v; memcpy(&v, data, sizeof(v)); value = v; } else memcpy(&value, data, sizeof(value)); for (uint16_t i = 0; i < len; i++) output[i] = (uint8_t)(value >> ((len - i - 1) * 8)); return true; }
