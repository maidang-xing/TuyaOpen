...
TuyaOpen 负责项目配置、Kconfig、组件编译和 CLI 调度；Jieli SDK 继续负责最终链接脚本、厂商静态库、section/movable region 和固件打包。平台 `build_example.py` 接收 TuyaOpen 生成的 `build_param.config`，将 TuyaOpen 示例对象和头文件传入 Jieli 工程，最终把标准命名的 QIO artifact 写入 TuyaOpen 的 `.build/bin/`。

`platform_flash_bridge.py` 实现 `platform_flash(using_data, binfile, port, baud, boards_root, logger)`，将 TuyaOpen 的 `tos.py flash` 参数转换成 Jieli 的 Linux 烧录工具调用。缺少匹配工具链、postbuild 工具、license 或串口时，bridge 必须返回清晰错误，而不能回退到不匹配的 `tyutool`。
...
2. 在已安装匹配 Jieli 工具链和 postbuild 工具的环境中，`tos.py build` 返回 0，并生成标准 QIO artifact。
...
- 当前已验证的旧 Jieli 工具链能完成 AC79 编译/链接，但 791x postbuild 工具不完整；该缺口必须保留为显式环境检查。
